...
public class WormholeNativeDemoActivity extends Activity {
  ...
  private static int WORMHOLE_ROOTID = 100192;//作为业务id传给虫洞，一个页面一个id,前端会根据该业务id来回收该id下面的所有虫洞view
  ...
  protected void initData() {
    for (int i = 0; i < 70; i++) {
      try {
        ...
        if (i == 3 || i == 11 || i == 19 || i == 23 || i == 31 || i == 37 || i == 43 || i == 49 || i == 52 || i == 61) {
  ...
  {
    // 销毁虫洞节点
    HippyWormholeManager.getInstance().sendRootDeleteMessageToClient(WORMHOLE_ROOTID);
    super.onDestroy();
  }
...
//          HippyWormholeView child = (HippyWormholeView) ((ViewGroup) viewHolder.itemView).getChildAt(0);
          ((ViewGroup) viewHolder.itemView).removeAllViews();
//          HippyWormholeManager.getInstance().onNativeWormholeItemDestroy(child,WORMHOLE_ROOTID);
        }
        HippyWormholeManager.getInstance().onNativeBindItemView(getWormholeId(position), viewHolder.itemView, getWormholeHippyMap(position),WORMHOLE_ROOTID);
      } else {
        //native
