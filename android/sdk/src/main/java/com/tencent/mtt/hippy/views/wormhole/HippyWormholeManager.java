...
public class HippyWormholeManager {
  ...
  public void onServerBatchComplete(HippyWormholeView wormholeView) {
    ...
    } else if (mNativeWormholeParentViewMap.containsKey(wormholeId)) {
      //从native场景里找
      View parent = mNativeWormholeParentViewMap.get(wormholeId);
      if (parent != null) {
        addWormholeToParent(wormholeView, parent);
        //将虫洞的宽高同步给parent
        resetParentHeight(wormholeView, parent);
        sendBatchCompleteMessageToClient(wormholeId, parent);
      }
    }
  }
  ...
  public HippySessionView findSessionViewByRootId(Integer rootId) {
    HippySessionView sessionView = mSessionViewMap.get(rootId);
    return sessionView;
  }
  ...
  private void resetParentHeight(View child, View parent) {
    ViewGroup.LayoutParams layoutParams = parent.getLayoutParams();
    if (layoutParams != null && layoutParams.height != child.getHeight()) {
      layoutParams.height = child.getHeight();
      parent.setLayoutParams(layoutParams);
    }
  }

  private void addNativeWormholeParent(String wormholeId, View parent) {
    mNativeWormholeParentViewMap.put(wormholeId, parent);
  }

  public void addNativeWormholeChild(String wormholeId, View child) {
    if (!TextUtils.isEmpty(wormholeId) && !mNativeWormholeChildViewMap.containsKey(wormholeId) && mNativeWormholeParentViewMap.containsKey(wormholeId)) {
      mNativeWormholeChildViewMap.put(wormholeId, child);
    }
  }

  private View getNativeWormholeChild(String wormholeId) {
  ...
  public void onNativeBindItemView(String wormholeId, View itemView, HippyMap hippyMap, int rootId) {
    ...
    if (childView != null) {
      ...
      HippyWormholeManager.getInstance().sendDataReceivedMessageToServer(hippyMap, rootId);
    }
  }

  public void onNativeWormholeDestroy(String wormholeId, int rootId) {
    if (!TextUtils.isEmpty(wormholeId) && mNativeWormholeParentViewMap.get(wormholeId) instanceof ViewGroup) {
      ...
      if (targetView.getChildCount() > 0) {
        ...
        if (child instanceof HippyWormholeView) {
          ...
          HippySessionView sessionView = findSessionViewByRootId(rootId);
          if (sessionView != null && mWormholeEngine != null) {
            sessionView.addView(child);
            HippyEngineContext engineContext = mWormholeEngine.getEngineContext();
            if (engineContext != null) {
      ...
      mNativeWormholeChildViewMap.remove(wormholeId);
    }
  }

  public void onNativeWormholeItemDestroy(HippyWormholeView child, int rootId) {
    if (child != null) {
      HippySessionView sessionView = findSessionViewByRootId(rootId);
      if (sessionView != null && mWormholeEngine != null) {
        sessionView.addView(child);
      ...
      mNativeWormholeParentViewMap.remove(child.getWormholeId());
      mNativeWormholeChildViewMap.remove(child.getWormholeId());
    }
  }
