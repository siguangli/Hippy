...
void RootNode::ReleaseResources() { animation_manager_->RemoveVSyncEventListener(); }
...
void RootNode::HandleEvent(const std::shared_ptr<DomEvent>& event) {
  ...
  auto js_runner = dom_manager->GetJsTaskRunner();
  auto task = std::make_unique<Task>(std::move(func));
  js_runner->PostTask(std::move(task));
}
