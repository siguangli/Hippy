...
public class RenderManager {
    ...
    public void createNode(int rootId, int id, int pid, int index,
        ...
        parentNode.addPendingChild(node);
        ...
        for (int i = 0; i < infoList.size(); i++) {
            try {
                ...
                if (child instanceof ListItemRenderNode) {
                    LogUtils.e("maxli", "moveNode: id " + child.getId() + ", index " + info.index);
                }
                child.setIndex(info.index);
                parent.addChildToPendingList(child);
                if (child instanceof ListItemRenderNode) {
    ...
    public void moveNode(int rootId, int[] ids, int newPid, int oldPid, int insertIndex) {
        ...
        for (int i = 0; i < ids.length; i++) {
            ...
            if (node != null) {
                ...
                node.setIndex(i + insertIndex);
                newParent.addChildToPendingList(node);
            }
        }
